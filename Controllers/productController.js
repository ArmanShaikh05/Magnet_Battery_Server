...
export const updateproduct = async (req, res, next) => {
  try {
    ...
    const newsDoc = await Product.findById(id);
...
export const showAllProduct = async (req, res, next) => {
  ...
  try {
    ...
    if (!postsData) return next(new ErrorHandler("No Product Found", 404));
...
export const getSingleItemData = async (req, res, next) => {
    try {
      const id = req.params.id;
      const specificData = await Product.findById(id);
